...
class Client:
    def __init__(
        self, scheme: str, host: str, sessions: dict[str, dict[str, str]] = None
    ):
    ...
    async def get_students(self) -> list[Student]:
        ...
        for units_group in self.sessions:
            ...
            self.http_session.session.cookie_jar.update_cookies(
                self.sessions[units_group]
            )
            ...
            permissions = Permissions(
                json.loads(
                    b64decode(get_script_param(start_page, "permissions").split("|")[0])
                )
            )
            for unit in permissions.units:
                ...
                request_verification_token: str = get_script_param(
                    student_start, "antiForgeryToken"
                )
                ...
                raw_registers = [
                    RawRegister(raw_register)
                    for raw_register in await self.http_session.student_request(
                        self.scheme,
                        self.host,
                        units_group,
                        unit.symbol,
                ...
                for raw_register in raw_registers:
                    cookies: dict[str, str] = {
                        ...
                        "idBiezacyDziennikPrzedszkole": str(
                            raw_register.kindergarten_register_id
                        ),
                        "idBiezacyDziennikWychowankowie": str(
                            raw_register.pupils_register_id
                        ),
                    ...
                    if not [
                        ...
                        if student.id == raw_register.student_id
                        and student.unit.id == unit.id
                        and student.unit.group == units_group
                    ]:
                        raw_school_data: RawSchoolData = RawSchoolData(
                            await self.http_session.student_request(
                                self.scheme,
                                self.host,
                                units_group,
